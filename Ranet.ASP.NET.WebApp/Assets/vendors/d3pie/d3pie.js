...
var helpers = {
    ...
    calculateLineWidth: function(string) {
        if (this._testArea === void(0)) {
            this._testArea = d3.select("body")
                .append("svg")
                .style("top", "" +
                    "-10000px" +
                    "")
                ...
                .append("text")
                .style("font-family", defaultSettings.labels.mainLabel.font).style("font-size", defaultSettings.labels.mainLabel.fontSize);
        }
        this._testArea.text(string);

        var bbox = this._testArea.node().getBBox();

        this._testArea.remove();
        return bbox.width;
    },

    getIdealLine: function (string, width, minLength, maxLength) {
        if (width <= 0) return "";
        if (maxLength === void(0)) maxLength = string.length;
        var desiredString;
        var calculatedWidth;
        if (minLength === void(0)) {
            desiredString = string;
            
            calculatedWidth = this.calculateLineWidth(desiredString);
            ...
            var length = Math.floor((minLength + maxLength) / 2);
            desiredString = string.slice(0, length) + "...";
            calculatedWidth = this.calculateLineWidth(desiredString);
        ...
        return this.getIdealLine(string, width, minLength, maxLength);
    },
...
var labels = {
	...
	positionLabelElements: function (pie, section, sectionDisplayType) {
	    console.log(sectionDisplayType);
		...
		switch (sectionDisplayType) {
			...
			case "label-percentage2":
				d3.selectAll("." + pie.cssPrefix + "segmentPercentage-" + section)
					...
					.attr("dy", function(d, i) { return dims[i].mainLabel.height; });
				break;
		    case "value-percentage1":
		        d3.selectAll("." + pie.cssPrefix + "segmentPercentage-" + section)
					.attr("dx", function (d, i) { return dims[i].value.width + singleLinePad; });
		        break;
		    case "value-percentage2":
		        d3.selectAll("." + pie.cssPrefix + "segmentPercentage-" + section)
					.attr("dx", function (d, i) { return (dims[i].value.width / 2) - (dims[i].percentage.width / 2); })
					.attr("dy", function (d, i) { return dims[i].value.height; });
		        break;
	 	}
	},
	...
	computeOuterLabelCoords: function(pie) {
		...
		pie.svg.selectAll("." + pie.cssPrefix + "labelGroup-outer")
			.each(function (d, i) {
			    labels.getIdealOuterLabelPositions(pie, i);
			    return labels.drawLabelBackground(pie, i);
			});
	...
	getIdealOuterLabelPositions: function(pie, i) {
		...
		var originalY = pie.pieCenter.y - (pie.outerRadius + 20);
        ...
        //Trying to copy devExtreme behaviour
		var svg = pie.svg.node();
		var bRect = svg.getBoundingClientRect();

		var outerLabelGroup = pie.outerLabelGroupData[i];
	    var textNode;
	    var maxWidth;
	    if (outerLabelGroup.hs === "right") {
	        maxWidth = bRect.width - newCoords.x;
	        if (outerLabelGroup.w > maxWidth) {
                
	            textNode = labelGroupNode.childNodes[0];
	            if (textNode) {
	                textNode.textContent = helpers.getIdealLine(textNode.textContent, maxWidth);
	                pie.outerLabelGroupData[i].w = textNode.getBBox().width;
	            }
	        }

	        pie.outerLabelGroupData[i].x = bRect.width - pie.outerLabelGroupData[i].w;
	    } else if (outerLabelGroup.hs === "left") {
	        maxWidth = outerLabelGroup.x - 2;
	        if (outerLabelGroup.w > maxWidth) {
	            textNode = labelGroupNode.childNodes[0];
	            if (textNode) {
	                textNode.textContent = helpers.getIdealLine(textNode.textContent, maxWidth);
		    ...
		    pie.outerLabelGroupData[i].x = 3;
		}
	},

	drawLabelBackground: function(pie, i) {
        var labelGroup = d3.select("#" + pie.cssPrefix + "labelGroup" + i + "-outer");
        
	    var textSize = labelGroup.node().getBBox();
            var padding = 3;
...
var segments = {
	...
	addSegmentEventHandlers: function(pie) {
		...
		arc.on("contextmenu", function (d, i) {
		    d3.event.preventDefault();
		    pie.options.callbacks.onContextSegment(this, d, d3.event);
		});
